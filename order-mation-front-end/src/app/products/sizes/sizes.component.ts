...
import { SizePopupComponent } from './size-popup/size-popup.component';
...
export class SizesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getSizes({ startCount: this.start, endCount: this.end });
  }
  searchActionTypes(params) {
    this.data = [];
    this.start = 1;
    this.end = 50;
    this.getSizes({ startCount: this.start, endCount: this.end, ...params });
  }
  newDialog(id = null) {
    const DialogRef = this.dialog.open(SizePopupComponent, {
      data: { id: id },
      width: '80%',
      minHeight: '70%',
      disableClose: true,
  ...
  getSizes(params) {
    const sub = this.sizeService.getSizes(params).subscribe({
      next: (res) => {
        this.data = res['data'];
  ...
  sortData(sort: Sort) {
    this.data = this.sortService.getSortedData(
      sort,
      ['sizeType', 'code', 'name', 'description', 'sortOrder'],
      this.data
    );
  }
