import { Component, OnInit, Optional } from '@angular/core';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { MatDialog, MatDialogRef } from '@angular/material/dialog';
import { Sort } from '@angular/material/sort';
import { ToastrService } from 'ngx-toastr';
import { filter, switchMap } from 'rxjs';
import { MessageService } from 'src/_services/message.service';
import { PricingService } from 'src/_services/rest/pricing.service ';
import { SortService } from 'src/_services/sort.service';
import { NewComponent } from './new/new.component';

@Component({
  selector: 'app-extraprice',
  templateUrl: './extraprice.component.html',
  styleUrls: ['./extraprice.component.scss'],
})
export class ExtrapriceComponent implements OnInit {
  data: [] = [];
  count: number = 0;
  start: number = 1;
  end: number = 50;
  search: FormGroup;
  constructor(
    @Optional() private dialogRef: MatDialogRef<NewComponent>,
    private dialog: MatDialog,
    private formBuilder: FormBuilder,
    private pricingService: PricingService,
    private messageService: MessageService,
    private toaster: ToastrService,
    private sortService: SortService
  ) {
    this.search = this.formBuilder.group({
        search: ['']
      })
  }

  ngOnInit(): void {
    this.getExtraPrices({ startCount: this.start, endCount: this.end });
  }
  newDialog(id = null) {
    const DialogRef = this.dialog.open(NewComponent, {
      data: { id: id },
      width: '80%',
      minHeight: '0%',
      disableClose: true,
    });
    const sub = DialogRef.afterClosed().subscribe((res) => {
      this.getExtraPrices({ startCount: this.start, endCount: this.end });
      sub.unsubscribe();
    });
  }
  searchActionTypes(params) {
    this.data = [];
    this.start = 1;
    this.end = 50 ;
    this.getExtraPrices({ startCount: this.start, endCount: this.end, ...params });
  }
  getExtraPrices(params=null) {
    const sub = this.pricingService.getExtraPrices(params).subscribe({
      next: (res) => {
        this.data = res['data'];
        this.count = res ['count']
        sub.unsubscribe();
      },
      error: (res) => {
        sub.unsubscribe();
      },
    });
  }

  deleteExtraPrices(id: number) {
    const isub = this.messageService
      .prompt(`Are you sure you want to delete this record?`)
      .afterClosed()
      .pipe(
        filter((_confirmed: boolean) => _confirmed),
        switchMap(() => {
          return this.pricingService.delete(id);
        })
      )
      .subscribe({
        next: (res) => {
          this.getExtraPrices({ startCount: this.start, endCount: this.end });
          this.toaster.success('Deleted Successfully', 'Deleted');
          isub.unsubscribe();
        },
        error: (res) => {
          isub.unsubscribe();
        },
      });
  }

  sortData(sort: Sort) {
    this.data = this.sortService.getSortedData(sort, [
      "code",
      "description",
      "unit",
      "price",
      ], this.data);
  }
  getPrevious() {
    this.start = this.start - 50;
    this.end = this.end - 50;
    this.getExtraPrices({
      query: this.search.value.search,
      startCount: this.start,
      endCount: this.end,
    });
  }

  getNext() {
    this.start = this.end;
    this.end = this.end + 50;
    this.getExtraPrices({
      query: this.search.value.search,
      startCount: this.start,
      endCount: this.end,
    });
}
}
